...
module.exports = function(logger, portalConfig, poolConfigs){
   ...
   this.getBlocks = function (cback) {
    ...
    let totalBlocks = [];
    let totalDiffi = [];
    let totalNetworkHashes = [];

    Object.keys(poolConfigs).forEach(function(coin){
        poolConfig = poolConfigs[coin];
        console.log(poolConfig.daemons[0].host + ':' + poolConfig.daemons[0].port);
        
        curl.setBody('{"jsonrpc": "1.0", "id":"curltest", "method": "getmininginfo", "params": [] }').post('http://' + poolConfig.daemons[0].user + ':' + poolConfig.daemons[0].password + '@' + poolConfig.daemons[0].host + ':' + poolConfig.daemons[0].port).then(({statusCode, body, headers}) => {
            totalBlocks[coin] = body.result.blocks;
            totalDiffi[coin] = Math.round(body.result.difficulty);
            totalNetworkHashes[coin] = Math.round(body.result.networkhashps);
        })
        .catch((e) => {
            console.log(e);
        });
                    ...
                    for(var i = 0; i < replies.length; i += commandsPerCoin){
                        ...
                        var coinStats = {
                            ...
                            poolStats: {
								...
								networkBlocks: totalBlocks[coinName],
								networkSols: totalNetworkHashes[coinName], 
								networkSolsString: getReadableNetworkHashRateString(totalNetworkHashes[coinName]), 
								networkDiff: totalDiffi[coinName],
